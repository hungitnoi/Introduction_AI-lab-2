...
class Solution():
    ...
    def write_image(self, file_name="state_space.png"):
        try:
            for node in self.graph.get_nodes():
                if node.get_label():
                    # Escape special characters and ensure proper quoting
    ...
    def draw_legend(self):
        """
        # utility function to draw legend on graph if legend flag is ON
        """
        graph_legend = pydot.Cluster(graph_name="legend", label="Legend", fontsize="20", color="gold",
                                   fontcolor="blue", style="filled", fillcolor="#fafd4a")
                                   
        node1 = pydot.Node("L1", style="filled", fillcolor="blue", label="Start Node", fontcolor="white", width="2", fixedsize="true")
        graph_legend.add_node(node1)
        
        node2 = pydot.Node("L2", style="filled", fillcolor="red", label="Killed Node", fontcolor="black", width="2", fixedsize="true")
        graph_legend.add_node(node2)
        
        node3 = pydot.Node("L3", style="filled", fillcolor="yellow", label="Solution nodes", width="2", fixedsize="true")
        graph_legend.add_node(node3)
        
        node4 = pydot.Node("L4", style="filled", fillcolor="grey", label="Can't be expanded", width="2", fixedsize="true")
        graph_legend.add_node(node4)
        
        node5 = pydot.Node("L5", style="filled", fillcolor="green", label="Goal node", width="2", fixedsize="true")
        graph_legend.add_node(node5)
       
        node7 = pydot.Node("L7", style="filled", fillcolor="gold", label="Node with child", width="2", fixedsize="true")
        graph_legend.add_node(node7)
        
        description = "Each node (m, c, s) represents a \nstate where 'm' is the number of\n missionaries,\n'c' the cannibals \
        \n and 's' is the side of the boat\n" \
        + "where  s == 1 represents left side and '0' the right side.\nOur goal is to reach state (0, 0, 0)\
        \nfrom start state (3, 3, 1) by some \noperators = [(0, 1), (0, 2), (1, 0), (1, 1), (2, 0)]\n"\
        + "\nwhere tuples (x, y) inside operators represents the number of missionaries and\
        \ncannibals to be moved from left to right \nif c == 1 and viceversa"
        
        node6 = pydot.Node("G", style="filled", fillcolor="gold", label= description, shape="plaintext", fontsize="20", fontcolor="red")
        graph_legend.add_node(node6)
        
        self.graph.add_subgraph(graph_legend)
    ...
    # And find parent until start state is reached
        ...
        for i, (number_missionaries, number_cannibals, side) in enumerate(steps):
                ...
                #     node.set_fillcolor("yellow")   ###Error
