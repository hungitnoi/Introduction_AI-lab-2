...
#set it in bin folder of graphviz
os.environ["PATH"] += os.pathsep + 'C:/Program Files (x86)/Graphviz/bin'

#dictionary to store backtrack solution node
#Parent stores the parent of (m,c,s)
#Move stores (x,y,side) i.e number of missionaries, cannibals 
#to be moved from left to right or right to left for particular state
#node list stores pydot.node object for particular state (m,c,s) so that we can color the solution nodes
...
class Solution():
    ...
    def write_image(self,file_name ="state_space.png"):
        ...
        print(f"File {file_name} is written successfully")
    ...
    def solve(self , solve_method = "dfs"):
        ...
        return self.dfs(*self.start_state, 0) if solve_method == "dfs" \
            else self.bfs()

    def draw_legend(self):
        """
            utility method to draw legend of graph if legend flag is on
        """
        ...
        node4= pydot.Node("4", style="filled", fillcolor="gray", label="Cant be expanded", \
                            width="2", fixedsize="true")
        ...
        node6= pydot.Node("6", style="filled", fillcolor="gold", label=description, shape='plaintext', \
    ...
    def draw(self, number_missionaries_left, number_cannibals_left, number_missionaries_right, number_cannibals_right):
        ...
        print('{}{}{}{}{}'.format(left_m, left_c, " " *(14 - len(left_m) - len(left_c)), right_m, right_c)),"_" * 40, " " * (12- len(right_m) - len(right_c))   
        print("")

    def show_solution(self):
        #recursively start from goal state 
        #and find parent until we reach start state
        ...
        for i, ((number_missionaries,number_cannibals,side),node) in enumerate(zip(steps[1:],nodes[1:])):
            ...
            print(f"Step {i+1} : Move {number_missionaries} missionaries and {number_cannibals} cannibals\
                   from {self.boat_side[side]} to {self.boat_side[int(not side)]}")
